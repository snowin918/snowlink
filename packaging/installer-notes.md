...
Optional portable archive produced after a successful GUI build:

```text
packaging/dist/Snowlink-MVP-portable.zip
```
...
- Bundle **WinRT** packages (`winrt-runtime`, `winrt-Windows.Graphics.Capture`, …)
  for DXcam `backend=winrt`. If omitted, the app falls back to DXGI at runtime.
